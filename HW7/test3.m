...
poly=conv([1,-0.86],conv([1,-0.82],conv([1,-0.5+0.5i],[1,-0.5-0.5i])));%pole assignment
A=[1 0 0 0;a1 1 b1 0;a2 a1 b2 b1;0 a2 0 b2];
b=[poly(2)-a1;poly(3)-a2;poly(4);poly(5)];
x=inv(A)*b;
alpha1=x(1);alpha2=x(2);beta0=x(3);beta1=x(4);

num=conv([b1,b2],[beta0,beta1]);%C(z)*G(z)
den=conv([1,a1,a2],[1,alpha1,alpha2]);%C(z)*G(z)
fb_num=num;%equivalent open loop sys.
fb_den=[0,0,num]+den;%equivalent open loop sys.
...
%     u(k+3)=sin(6*(k+2)/20)+0.5*cos(6*(k+2)/15+3.2)+0.2*sin(2.57*(k+2)/13+1.36);
    y(k+4)=-fb_den(2)*y(k+3)-fb_den(3)*y(k+2)-fb_den(4)*y(k+1)-fb_den(5)*y(k)+fb_num(1)*u(k+2)+fb_num(2)*u(k+1)+fb_num(3)*u(k)+d(k+3);
end 
figure(1);
plot(y);
hold on;
